...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runCal := (runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
	return runCal
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedMetInSec := meanSpeed(action, duration) * kmhInMsec
	walkCal := (walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMetInSec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
	return walkCal
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimCal := (speed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimCal
}
